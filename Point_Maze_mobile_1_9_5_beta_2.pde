...
int s;
int cRadius;
//File saveExisting = dataFile("save.txt");
PImage a;
PImage b;
PImage c;
...
void draw()
{
  ...
  if (start == true)
  {
    ...
    textAlign(CENTER, CENTER);
    text("Tippen zum starten", width/2, height/2);
  ...
  {
    imageMode(CENTER);
    image(a, width - 150, 50, 60, 60);
    imageMode(CORNER);
    noFill();
    stroke(255);
    strokeWeight(3);
    ellipse(width - 150, 50, 100, 100);
  }
...
void win()
{
  ...
  if(win > 115 || win < 110 && win > 105)
  {
    if (vibrate == true)
    {
      ((Vibrator) getContext().getSystemService(Context.VIBRATOR_SERVICE)).vibrate(40);
    }
  }
}
...
void mousePressed()
{
  ...
  if (pause == true)
  {
    if(mouseX > width - 200 && mouseX < width - 100 && mouseY > 0 && mouseY < 100)
    {
      settings = !settings;
    }
    ...
    if (settings == true)
    {
      if (mouseX > 260 && mouseX < 330 && mouseY < 295 && mouseY > 145)
      {
        vibrate = !vibrate;
        if (vibrateSwitch == 180)
        {
          vibrateSwitch = 260;
        }
        else
